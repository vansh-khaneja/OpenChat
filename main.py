...
from utils import execute_query, rephrase_ans, find_order_id,guard_check,fetch_all_data,fetch_api_data,is_list
...
The issue_query is for any query related to unresolved issues or when users say pass this to the support team or say this issue is not yet resolved or not clear or if user is simply providing numner it means its for the issue query .
...
def generate_ans(question,api_key,chat_history,user_id):
    inputs = {"question": question,"api_key":api_key,"chat_history":chat_history,"user_id":user_id}#
    ...
    if "Llama Guard" not in advance_features:
        return answer
    else:
        if(guard_check(answer)=="safe"):
            return answer
        else:
            return "Some voilence found in text generation"
...
def echo_text():
    ...
    ans = generate_ans(input_text, api_key,chat_history,user_id)  # Ensure generate_ans is not async in Flask
...
def echo():
    ...
    input_url = request.json.get("urls")

    # if(is_list(input_url)):
    text_from_list = train_data.get_text_from_urls(input_url)
    overall_text = text_from_list +"\n" + input_text
    
    db_name = train_data.db_gen(overall_text)
