...
def user_data_retrieve(state):
    ...
    print(user_id)
    inc_insights("user_data_insights",api_key)
    data = fetch_all_data()
    ...
    # Retrieval
    ans = fetch_user(question+f" (user id is {user_id})",postgresUrl)
    answer = rephrase_ans(question, execute_query(ans,postgresUrl) + f"this is response from the database and this is th chat history if needed to get context {str(chat_history)}",api_key,str(chat_history))
